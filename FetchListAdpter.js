 ...
 * 【使用】通过传递简单的数据配置，快速完成 接口请求+字段赋值+打包返回预期数据 的工作（没错，老一条龙了）
...
export function fetchListAdpter(fetchConfig = {
  fetchExecutor: () => Promise.resolve(null), //必须滴：【一个获取接口的Promise方法，通常我们都会承诺些什么】
  ...
  return new Promise(async (resolve, reject) => {
    try {
      if (typeof params != 'object' || Array.isArray(params)) {
        ...
        return reject(new Error('params必须为Object类型'));
      }

      if (typeof fetchExecutor != 'function') {
        fetchConfig.fetchExecutor = () => Promise.resolve(null); //修正请求方法
        return reject(new Error('fetchExecutor必须为Promise类型的方法'));
      }

      let res = await fetchExecutor(params);

      if (!res) {
        return reject(new Error(`接口返回数据和预期不符，请修改传递的配置参数。接口返回如下：${JSON.stringify(res)}`));
      } else {
        // 处理列表数据
        ...
        if (dataItemDecorator) {
          if (typeof dataItemDecorator == 'function') {
            ...
            return reject(new Error('dataItemDecorator必须为Function类型，且接受一个参数'));
          }
        }
        ...
        // 处理附加字段 (用户自行配置)
        if (extraAdpter && !Array.isArray(extraAdpter)) {
          return reject(new Error("extraAdpter必须为对象数组，形如：[{dataKey:'xx', dataKeyChain:'xx.xx.xx'}]"));
        }
        extraAdpter.length && extraAdpter.map(({
          dataKey,
          dataKeyChain,
          dataDecorator
        }) => {
